...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="按日期区间生成生产训练集 train.csv 和历史验证样本 test.csv"
	)
	parser.add_argument(
		"--input",
		type=str,
	...
	parser.add_argument(
		...
		type=str,
		default="2024-04-09",
		help="训练集开始日期，默认 2024-04-09",
	)
	parser.add_argument(
		"--train-end",
		type=str,
		default="2026-06-26",
		help="生产训练集结束日期，默认 2026-06-26",
	)
	parser.add_argument(
		"--test-start",
		type=str,
		default="2026-06-15",
		help="历史验证样本开始日期，默认 2026-06-15",
	)
	parser.add_argument(
		"--test-end",
		type=str,
		default="2026-06-18",
		help="历史验证样本结束日期，默认 2026-06-18。端午休市时T+5复用T+4，不顺延到6月22日",
	)
...
def main() -> None:
	...
	if train_end >= test_start:
		print(
			"提示: train.csv 为生产全量训练集，test.csv 仅作历史验证样本，预测链路不读取 test.csv。"
		)
