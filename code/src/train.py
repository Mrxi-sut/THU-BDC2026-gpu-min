...
class SoftPortfolioReturnLoss(nn.Module):
    """
    可微的软组合收益损失。

    用模型分数在当前横截面里形成 softmax 权重，并最大化这组权重对应
    的未来收益。top_k 用于让目标更贴近提交时只买少数股票的决策。
    """
    def __init__(self, temperature=0.25, top_k=10):
        super(SoftPortfolioReturnLoss, self).__init__()
        self.temperature = max(float(temperature), 1e-6)
        self.top_k = top_k

    def forward(self, scores, future_returns):
        if self.top_k is not None and self.top_k > 0 and scores.numel() > self.top_k:
        ...
        portfolio_return = torch.sum(weights * future_returns)
...
# 排序训练函数
def train_ranking_model(model, dataloader, criterion, decision_criterion, optimizer, device, epoch, writer):
    model.train()
    total_loss = 0
    ...
    total_metrics = {}
    local_step = 0
    decision_loss_weight = float(config.get('decision_loss_weight', 0.0) or 0.0)
    ...
    for batch in tqdm(dataloader, desc=f"Training Epoch {epoch+1}"):
        ...
        for i in range(batch_size):
            ...
            valid_future_return = masked_targets[i][valid_indices]
            
            if len(valid_pred) > 1:
                # 直接使用预处理好的相关性得分，无需重新计算
                ranking_loss = criterion(valid_pred.unsqueeze(0), valid_relevance.unsqueeze(0))
                if decision_loss_weight > 0:
                    decision_loss = decision_criterion(valid_pred, valid_future_return)
                    loss = ranking_loss + decision_loss_weight * decision_loss
        ...
        if batch_loss is not None:
            ...
            if writer:
                ...
                if loss_component_count > 0:
                    writer.add_scalar('train/ranking_loss', total_ranking_loss / loss_component_count, global_step=epoch*len(dataloader)+local_step)
                    writer.add_scalar('train/decision_loss', total_decision_loss / loss_component_count, global_step=epoch*len(dataloader)+local_step)
                for k, v in metrics.items():
    ...
    if loss_component_count > 0:
        total_metrics['ranking_loss'] = total_ranking_loss / loss_component_count
        total_metrics['decision_loss'] = total_decision_loss / loss_component_count

    return total_loss / len(dataloader) if len(dataloader) > 0 else 0, total_metrics

def evaluate_ranking_model(model, dataloader, criterion, decision_criterion, device, writer, epoch):
    model.eval()
    total_loss = 0
    total_ranking_loss = 0
    total_decision_loss = 0
        ...
        for batch in tqdm(dataloader, desc=f"Evaluating Epoch {epoch+1}"):
            ...
            for i in range(batch_size):
                ...
                if len(valid_pred) > 1:
                    ...
                    ranking_loss = criterion(valid_pred.unsqueeze(0), relevance_scores.unsqueeze(0))
                    if decision_loss_weight > 0:
                        decision_loss = decision_criterion(valid_pred, valid_true)
                        loss = ranking_loss + decision_loss_weight * decision_loss
                        total_decision_loss += decision_loss.item()
                    else:
                        loss = ranking_loss
...
def main():
    ...
    decision_criterion = SoftPortfolioReturnLoss(
        temperature=config.get('decision_temperature', 0.25),
        top_k=config.get('decision_top_k', 10)
    )
    print(
        "Decision loss: "
        f"weight={config.get('decision_loss_weight', 0.0)}, "
        f"temperature={config.get('decision_temperature', 0.25)}, "
        f"top_k={config.get('decision_top_k', 10)}"
    )
    ...
    if is_train:
        ...
        for epoch in range(config['num_epochs']):
            ...
            train_loss, train_metrics = train_ranking_model(
                model, train_loader, criterion, decision_criterion, optimizer, device, epoch, writer
            )
            ...
            eval_loss, eval_metrics = evaluate_ranking_model(
                model, val_loader, criterion, decision_criterion, device, writer, epoch
            )
        ...
        if config.get('tabular_ranker_enabled', True):
            # 表格排序器是独立增强分支，不覆盖 Transformer 的 best_model.pth。
            # 即使增强分支失败，原始深度模型仍可正常预测，方便做消融对比。
            try:
                tabular_metrics = train_tabular_ranker(full_df, output_dir, config)
                with open(os.path.join(output_dir, 'final_score.txt'), 'a', encoding='utf-8') as f:
                    f.write(
                        "Tabular daily_top5_mean_return: "
                        f"{tabular_metrics.get('daily_top5_mean_return', 0.0):.6f}\\n"
                    )
                    f.write(
                        "Tabular daily_top5_positive_ratio: "
                        f"{tabular_metrics.get('daily_top5_positive_ratio', 0.0):.6f}\\n"
                    )
                print(
                    "表格排序增强完成 - "
                    f"daily_top5_mean_return: {tabular_metrics.get('daily_top5_mean_return', 0.0):.6f}, "
                    f"positive_ratio: {tabular_metrics.get('daily_top5_positive_ratio', 0.0):.4f}"
                )
            except Exception as exc:
                print(f"表格排序增强训练失败，预测阶段将回退到 Transformer: {exc}")
